...
def build_last_layer_from_output_dist(dim,input_size, output_type):
        ...
        if output_type == "bernoulli" or output_type == "poisson" or\
            output_type == "B" or output_type == "P":
...
class Decoder(nn.Module):
    ...
    # define the forward computation on the latent z
        ...
        for k in range(self.n_inputs):
            print(self.layers_independent_input)
            for lay in range(self.n_layers_ind + 1):
                if lay == 0:
...
output_types_input = ["G"]

dec = Decoder(INPUT_SIZE, z_dim, hidden_dims_dec_common,hidden_dims_dec_last , output_types_input)

input = torch.ones((3000,30))
